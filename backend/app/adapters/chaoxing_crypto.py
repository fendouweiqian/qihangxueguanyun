"""超星登录参数使用的 SM4-CBC 加密实现。"""

from __future__ import annotations

import base64


_KEY = b"90ACB357C1AC99D4"
_IV = b"5BD730C485F2AF10"
_SBOX = [
    ,
]
_FK = [0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC]
_CK = [
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269, 0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249, 0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229, 0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209, 0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279,
]


def _u32(value: int) -> int:
    """按 JavaScript 位运算规则截断为有符号 32 位整数。"""
    value &= 0xFFFFFFFF
    return value - 0x100000000 if value & 0x80000000 else value


def _rotl(value: int, shift: int) -> int:
    """执行 32 位循环左移。"""
    signed = _u32(value)
    return _u32(_u32(signed << shift) | _u32(signed >> (32 - shift)))


def _tau(value: int) -> int:
    """执行 SM4 S 盒替换。"""
    return (_SBOX[(value >> 24) & 0xFF] << 24) | (_SBOX[(value >> 16) & 0xFF] << 16) | (_SBOX[(value >> 8) & 0xFF] << 8) | _SBOX[value & 0xFF]


def _round_keys(key: bytes) -> list[int]:
    """生成 SM4 轮密钥。"""
    if len(key) != 16:
        raise ValueError("SM4 key must be 16 bytes")
    values = [int.from_bytes(key[index:index + 4], "big") for index in range(0, 16, 4)]
    values = [_u32(value ^ _FK[index]) for index, value in enumerate(values)]
    result: list[int] = []
    for index in range(32):
        mixed = _u32(values[index + 1] ^ values[index + 2] ^ values[index + 3] ^ _CK[index])
        substituted = _tau(mixed)
        transformed = _u32(substituted ^ _rotl(substituted, 13) ^ _rotl(substituted, 23))
        values.append(_u32(values[index] ^ transformed))
        result.append(values[index + 4])
    return result


def encrypt_cbc(data: bytes, key: bytes = _KEY, iv: bytes = _IV) -> bytes:
    """使用 SM4-CBC-PKCS7 加密字节串。"""
    if len(iv) != 16:
        raise ValueError("SM4 iv must be 16 bytes")
    padding = 16 - len(data) % 16
    data += bytes([padding]) * padding
    round_keys = _round_keys(key)
    output = bytearray()
    previous = iv
    for offset in range(0, len(data), 16):
        block = bytes(left ^ right for left, right in zip(data[offset:offset + 16], previous))
        words = [int.from_bytes(block[index:index + 4], "big") for index in range(0, 16, 4)]
        for index in range(32):
            mixed = _u32(words[index + 1] ^ words[index + 2] ^ words[index + 3] ^ round_keys[index])
            substituted = _tau(mixed)
            transformed = _u32(substituted ^ _rotl(substituted, 2) ^ _rotl(substituted, 10) ^ _rotl(substituted, 18) ^ _rotl(substituted, 24))
            words.append(_u32(words[index] ^ transformed))
        encrypted = b"".join((words[index] & 0xFFFFFFFF).to_bytes(4, "big") for index in (35, 34, 33, 32))
        output.extend(encrypted)
        previous = encrypted
    return bytes(output)


def encrypt_login(value: str | None) -> str:
    """生成超星登录字段的 Base64 文本。"""
    if value is None:
        return ""
    encoded = base64.b64encode(encrypt_cbc(value.encode("utf-8"))).decode("ascii")
    return encoded.replace("+", "$ShowMe$")
